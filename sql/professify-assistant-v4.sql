-- =================================================================================================
-- professify-assistant-v4.sql — 2026-09-23
-- Run AFTER professify-assistant-v2.sql. Supersedes v3 (running v3 first does no harm).
-- One transaction, idempotent.
--
-- One change: the tool whitelist in assistant_log_run now names every tool the function can emit —
-- 31, after "a tool for every tap" (watch, add_section, rate_professor, draft_message, share,
-- add_friend, friend_profile, professor_stats, compare_professors, open_section, set_theme, help,
-- plus v3's my_professors and friends_took). A tool missing here is answered but never logged, so
-- the weekly miss review would never see it. tests/check-ask-function.ts fails if this list and
-- the function's tools ever disagree.
-- =================================================================================================
begin;
...
begin
  ...
  if position('compare_professors' in src) = 0 or position('friends_took' in src) = 0
     or position('draft_message' in src) = 0 then raise exception 'v4: whitelist not updated'; end if;
  if position('SPEND FIRST' in src) = 0 then raise exception 'v4: lost spend-first'; end if;
end $$;
