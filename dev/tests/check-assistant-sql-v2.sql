-- v2 of the assistant SQL suite: the same assertions against the raised caps (40/hour, 150/day).
-- Run against a database with professify-assistant.sql AND professify-assistant-v2.sql applied.
...
begin
  ...
  perform pg_temp.ok(allowed = 40, format('the hourly ceiling is exactly 40 (v2), got %s', allowed));
  perform pg_temp.ok((public.assistant_gate()->>'reason') = 'hourly_cap',
    'the 41st call in an hour is refused as an hourly cap');
end $$;
...
begin
  ...
  perform pg_temp.ok(
    (select day_count from public.assistant_rate where user_id = '22222222-2222-2222-2222-222222222222') = 41,
...
begin
  ...
  -- stops us. This is testing the 150, not the 40.
  for i in 1..150 loop
    update public.assistant_rate
       set hour_start = date_trunc('hour', now()) - interval '2 hours'
     where user_id = '22222222-2222-2222-2222-222222222222';
  ...
  perform pg_temp.ok(
    (select day_count from public.assistant_rate where user_id = '22222222-2222-2222-2222-222222222222') = 150,
    'and it stops at exactly 150, not 151');
end $$;
